...
export default function Home() {
  ...
  const handleAnalyseAI = async (audio: AudioPayload) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.error || "An error occurred during audio analysis."
        );
      }

      setAnalysisResult(data as AudioAnalysisResponse);
    } catch (err: unknown) {
      const errObj = err as { message?: string };
      setAnalysisError(
        errObj.message || "Failed to analyze audio with AI service."
      );
    } finally {
      setIsAnalyzing(false);
    }
        ...
        {analysisResult && (
          <div className="rounded-2xl border border-indigo-200/90 bg-white/95 p-5 sm:p-6 shadow-sm space-y-4">
            <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
              ...
              <div>
                <h3 className="text-base font-semibold text-slate-900">AI Analysis Results</h3>
                <p className="text-xs text-slate-600">Generated by Gemini 2.5</p>
              </div>
            </div>
